...
function sirius_custom_css() {
    ...
    if($sirius_advanced_css != '') {    
        echo '<!-- Custom CSS -->';
        $output="<style>" . stripslashes($sirius_advanced_css) . "</style>";
        echo $output;
        echo '<!-- /Custom CSS -->';
    }
}
...
function filter_by_category( $query ) {
    ...
    $category_id = $categories[0]->cat_ID; //change to the actual name of the category you are filtering with
...
function sirius_rand_page() {
    ...
    //get_pages() slows down customizer load time in case of a large number of pages. "Sample Page" ID=2
    return 2; 
} 
